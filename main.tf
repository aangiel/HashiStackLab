...
module "vcn_subnet" {
  ...
  subnets = {
    subnet = {
      ...
      dns_label  = "subnet"
      security_list_ids = [
        module.vcn.vcn_all_attributes.default_security_list_id,
        oci_core_security_list.hashistack_sec_list.id
      ]
    }
  }
}
...
module "arm-compute-instance-1" {
  ...
  public_ip_display_name      = "hashi-arm-1"
...
module "arm-compute-instance-2" {
  ...
  ipxe_script                 = ""
  primary_vnic_nsg_ids        = []
  public_ip_display_name      = "hashi-arm-2"
  source_ocid                 = var.image_ocid
  ssh_public_keys             = file("./public-keys")
  subnet_ocids                = values(module.vcn_subnet.subnet_id)
